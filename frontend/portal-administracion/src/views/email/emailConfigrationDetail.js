...
const EmailConfigurationDetail=(props) =>{
    const row=props.row;
...
const ListDetail=(props) =>{
  ...
  useEffect(()=>{
    EmailConfigurationServices.GetEmailConfiguration({"TYPE":"SERVER"})
        .then((data)=>{
            setRows(data);
        });
  },[cont]);
          ...
          secondary={
            <React.Fragment>
              ...
              >
                ...
                {row.name}
                {!!row.server?" Servidor "+row.server:""}
                {!!row.port?" Puerto "+row.port:"" }
...
export default EmailConfigurationDetail;