...
function ProcessForm(props) {
    ...
    const onClick=()=>{
      ...
      ProcessInstanceServices.InitProcesses({"processName":workflowName,"data":dataForm.data}).then((data)=>
      {
        console.log(data);
      });
    }
    useEffect(()=>{
      ProcessFormServies.GetProcessesForm({"processName":processName}).then((rta)=>{
